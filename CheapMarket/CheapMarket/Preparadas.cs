...
namespace Diseño
{
    public partial class Preparadas : Form
    {
        ...
        private void CargarProductos()
        {
            ...
            string consulta = String.Format("SELECT Nombre, Precio, Descripcion FROM producto where Categoria='COMIDAS PREPARADAS'");

            if (ConexionBD.AbrirConexion())
            {
                dgvPreparadas.DataSource = Utilidades.CargarProductos2(ConexionBD.Conexion, consulta);
        ...
        private void btnAgregar_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                if (ConexionBD.AbrirConexion())
                {
                    if (Utilidades.ComprobarProducto(ConexionBD.Conexion, dgvPreparadas.CurrentRow.Cells[0].Value.ToString()))
                    {
                        ConexionBD.CerrarConexion();

                        string dni = Sesion.NifUsu;
                        string nombre = dgvPreparadas.CurrentRow.Cells[0].Value.ToString();
                        double precio = double.Parse(dgvPreparadas.CurrentRow.Cells[1].Value.ToString());
        ...
        private void pcbLupa_Click(object sender, EventArgs e)
        {
            if (ConexionBD.AbrirConexion())
            {
                dgvPreparadas.DataSource = Utilidades.FiltrarProductos(ConexionBD.Conexion, "COMIDAS PREPARADAS", txtBuscar.Text);
