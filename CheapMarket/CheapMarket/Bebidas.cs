...
namespace Diseño
{
    public partial class Bebidas : Form
    {
        ...
        private void CargarProductos()
        {
            ...
            string consulta = String.Format("SELECT Nombre, Precio, Descripcion FROM producto where Categoria='BEBIDAS'");

            if (ConexionBD.AbrirConexion())
            {
                dgvBebidas.DataSource = Utilidades.CargarProductos2(ConexionBD.Conexion, consulta);
        ...
        private void pcbLupa_Click(object sender, EventArgs e)
        {
            if (ConexionBD.AbrirConexion())
            {
                dgvBebidas.DataSource = Utilidades.FiltrarProductos(ConexionBD.Conexion, "BEBIDAS", txtBuscar.Text);
