...
namespace Diseño
{
    public partial class Fruteria : Form
    {
        ...
        private void CargarProductos()
        {
            ...
            {
                dgvFruteria.DataSource = Productos.CargarProductos2(ConexionBD.Conexion, consulta);
        ...
        private void dgvFruteria_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            string producto = dgvFruteria.CurrentRow.Cells[0].Value.ToString();
        ...
        private void btnAgregar_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                if (ConexionBD.AbrirConexion())
                {
                    if (Utilidades.ComprobarProducto(ConexionBD.Conexion, dgvFruteria.CurrentRow.Cells[0].Value.ToString()))
                    {
                        ConexionBD.CerrarConexion();

                        string dni = Sesion.NifUsu;
                        string nombre = dgvFruteria.CurrentRow.Cells[0].Value.ToString();
                        double precio = double.Parse(dgvFruteria.CurrentRow.Cells[1].Value.ToString());
        ...
        //Metodo para filtrar procutos
        private void pcbLupa_Click(object sender, EventArgs e)
        {
            if (ConexionBD.AbrirConexion())
            {
                dgvFruteria.DataSource = Productos.FiltrarProductos(ConexionBD.Conexion, "FRUTERIA", txtBuscar.Text);
