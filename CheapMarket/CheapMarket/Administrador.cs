...
namespace CheapMarket
{
    class Administrador
    {
        ...
        public int BorrarProducto(MySqlConnection conexion, string codigo)
        {
            string consulta;
            consulta = string.Format("DELETE FROM productos WHERE codigo = '{0}'",codigo);
            MySqlCommand comando = new MySqlCommand(consulta, conexion);
            return comando.ExecuteNonQuery();
        }
        ...
        /// <param name="categoria">Nueva categoria del producto</param>
        /// <returns>Número de registros afectados</returns>
        public int EditarProducto(MySqlConnection conexion, string codigo, string nombre, double precio, string descripcion, string categoria)
        {
            string consulta;
